...
class ApplicationRecoverAndDraftSubmitUseCaseSpec extends Specification  {

    void 'test application recover request and then draft use case '() {
        given: 'a ApiGatewayAdapter instance'
        def recoverApiGatewayAdapter = new ApplicantRecoverRequestApiGatewayAdapter()
        def draftSubmitApiGatewayAdapter = new DraftFormAnsweredApiGatewayAdapter()
        ...
        and: 'Submitted partial form sent'

        def draftFormApiGatewayAdapter = new DraftFormAnsweredApiGatewayAdapter()
        JsonFormAnswered formAnswered = getFormAnswered()
        draftFormApiGatewayAdapter.handleRequest(formAnswered, context)

        and: 'recovery key sent to user'
        RedisApplicationStorageAdapter applicationStorageAdapter = new RedisApplicationStorageAdapter()
        String applicantKey = formAnswered.getAnswers().stream().filter{answer -> answer.getQuestionId().equals("16")}.findFirst().get().getText();
        String tokenKey = applicationStorageAdapter.findKeys(applicantKey).stream().findFirst().get()
        String recoveryKey = applicationStorageAdapter.findByKey(tokenKey.substring(12)).getRecoveryKey();
        and: 'form request input message'
        def applicationRecoverRequest = new JsonApplicationRecoverWithRecoveryKeyRequested(
                "a8b6b59d-5e6d-4ac0-8000-b44f8d6a5ca8",
                "recoveranddraft@email.com",
                recoveryKey)



        when: 'api gateway adapter recieve a message by http'
        def response = recoverApiGatewayAdapter.handleRequest(applicationRecoverRequest, context);

        and: 'api gateaway adapter recieve a form submit request'

        def gson = new GsonBuilder().create()
        JsonFormAnswered draftFormAnswered = gson.fromJson(response, JsonFormAnswered.class)
        draftFormAnswered.getAnswers().removeIf({x -> x.getQuestionId().equals("5")})
        draftSubmitApiGatewayAdapter.handleRequest(draftFormAnswered, context)
    ...
    "answers": [
        ...
        {
            "questionId": "16",
            "text": "recoveranddraft@email.com"
        },
        {
            "questionId": "8",
            "choices": [
