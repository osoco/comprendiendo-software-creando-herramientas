...
class RecoverTokenUseCaseSpec extends Specification {

    void 'test recover token use case when applicant not exists'() {
        given: 'a ApiGatewayAdapter instance'
        CreateFixtureForm.createForm()

        and: 'form answered input message'
        JsonTokenRecoverRequested tokenRecoverRequested = new JsonTokenRecoverRequested("a8b6b59d-5e6d-4ac0-8000-b44f8d6a5ca8", "ES", "test@osoco.es")
        ...
        apiGatewayAdapter.handleRequest(tokenRecoverRequested, context)
    ...
    void 'test recover token use case where applicant exist'() {
        given: 'clean redis database'
        CleanRedisHelper.cleanRedis()
        CreateFixtureForm.createForm()

        and: 'form answered input message'
        JsonTokenRecoverRequested tokenRecoverRequested = new JsonTokenRecoverRequested("a8b6b59d-5e6d-4ac0-8000-b44f8d6a5ca8","ES","submit@email.com")
        ...
        and: 'Submitted partial form sent'
        def draftFormApiGatewayAdapter = new DraftFormAnsweredApiGatewayAdapter()
        JsonFormAnswered formAnswered = getDraftFormAnswered()
        draftFormApiGatewayAdapter.handleRequest(formAnswered, context)

        when: 'api gateway adapter receive a message by http'
        def response = apiGatewayAdapter.handleRequest(tokenRecoverRequested, context)

        then:
        response == Boolean.TRUE
    }

    RecoverTokenApiGatewayAdapter apiGatewayAdapter = new RecoverTokenApiGatewayAdapter()

    private JsonFormAnswered getFormAnswered() {
        gson.fromJson(formAnsweredJson, JsonFormAnswered.class)
    }

    private JsonFormAnswered getDraftFormAnswered() {
        gson.fromJson(formDraftAnsweredJson, JsonFormAnswered.class);
    }
    ...
    private final static formDraftAnsweredJson = '''{
    "contestId": "a8b6b59d-5e6d-4ac0-8000-b44f8d6a5ca8",
    "formId": "form-1",
    "language": "ES",
    "sections": [
