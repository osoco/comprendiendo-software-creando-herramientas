...
class ApplicantRecoverRequestApiGatewayAdapterSpec extends Specification {

    void 'test application recover request use case with incorrect token'() {
        given: 'a ApiGatewayAdapter instance'
        def apiGatewayAdapter = new ApplicantRecoverRequestApiGatewayAdapter()
        ...
        def applicationRecoverRequest = new JsonApplicationRecoverWithRecoveryKeyRequested(
                ...
                "recover@email.com",
                "0752119a-5c89-40d2-4652-ff8717f6df23")
        ...
        apiGatewayAdapter.handleRequest(applicationRecoverRequest, context)

        then:
        thrown ApplicationNotFoundException
    }

    void 'test application recover request use case with correct token'() {
        given: 'a ApiGatewayAdapter instance'
        def apiGatewayAdapter = new ApplicantRecoverRequestApiGatewayAdapter()
        CreateFixtureForm.createForm()
        ...
        String applicantKey = formAnswered.getAnswers().find{ it.questionId == "16"}.text
        String tokenKey = applicationStorageAdapter.findKeys(applicantKey).first()
        String recoveryKey = applicationStorageAdapter.findByKey(tokenKey.substring(12)).recoveryKey
        and: 'form request input message'
        def applicationRecoverRequest = new JsonApplicationRecoverWithRecoveryKeyRequested(
                "a8b6b59d-5e6d-4ac0-8000-b44f8d6a5ca8",
                "recover@email.com",
        ...
        def response = apiGatewayAdapter.handleRequest(applicationRecoverRequest, context)
    ...
    private JsonFormAnswered getFormAnswered() {
